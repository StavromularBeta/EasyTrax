class EasyTraxParse:
    ...
    the page, samples going down). Both data types can be handled together or independently - sample metadata
    is not present in vertically displayed files, so backup sample metadata is gathered from the report headers.
    ...
    def pre_generate_backup_samples_dictionary_entries(self):
        ...
        this if this is not the case in the future (09June21). empty_list is used to determine when we've come to a gap
        in the page, which means we are no longer reading in our sample information. Once a gap is hit, it's assumed
        that we've reached the end of the header. We then collapse all the lists of lines into one long list. We
        run through this list. The first time we hit a parenthesis ')', we know were at the start of a sample. Once
        we hit another parenthesis, we know we are at the start of the next sample. This logic is used to split the
        information up into segments where the last three items will be location code, date, and time, the first
        item will be the sample number, and the remaining indexes when combined in order are the sample name. """
    ...
    def generate_backup_samples_dictionary_entries(self):
        ...
        first looks at whether we have any samples dictionary keys already. There are two options that
        require backup metadata - either there are no horizontal tables, or there are samples that exist
        in vertical data tables that don't exist in subsequent horizontal data tables in the file. Checks
        to see if there are any backup sample indices to work on - sometimes, when there is only horizontal
        data in the report, the admin staff don't add in the samples line. Each sublist corresponds to a
        sample. The first two characters of the first item in the sublist will be the sample number if the
        sample is between 10-99. Otherwise, the sample will fail an int check because the second character
        will be a parenthesis, at which point we take just the first character. We then check if we already
        have information on the particular sample. If we don't, we make a new samples dictionary entry,
        and now the ICP data has somewhere to go. """
